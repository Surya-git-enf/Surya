...
export default function SplashCursor({
  ...
  useEffect(() => {
    ...
    function getUniforms(program: WebGLProgram): { [name: string]: WebGLUniformLocation } {
      ...
      let uniformCount = gl.getProgramParameter(program, gl.ACTIVE_UNIFORMS) as number;
    ...
    function resizeDoubleFBO(
      ...
      target.read = resizeFBO(target.read, w, h, internalFormat, format, type, param) as any;
      target.write = createFBO(w, h, internalFormat, format, type, param) as any;
      target.width = w;
      target.height = h;
    ...
    function drawColor(target: FBO | null, color: { r: number; g: number; b: number }) {
      clearProgram.bind();
      gl.uniform4f(clearProgram.uniforms.value as any, color.r, color.g, color.b, 1);
      blit(target);
    }
    ...
    canvas.addEventListener('touchstart', (e: TouchEvent) => {
      ...
      for (let i = 0; i < touches.length; i++) {
        ...
        pointers[i + 1].color = generateColor();
      }
    }, { passive: false });
