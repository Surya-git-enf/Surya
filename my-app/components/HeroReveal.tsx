...
export default function HeroReveal() {
  ...
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      // 2 — WALK: translate completely to the right edge (xPercent: -100 aligns it flush right)
      tl.to(charWrapRef.current, { left: "100%", xPercent: -100, duration: 2.4, ease: "none" }, "+=0.1");
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex flex-col items-stretch w-full px-[5vw]">
        {/* TOP ROW — PEDDISHETTI */}
        <div ref={topWordRef} className="flex justify-between items-baseline w-full" aria-label="PEDDISHETTI">
        ...
        >
          ...
          {/* ── Walking Legs (Now inside the text row to share the exact same baseline) ── */}
          <div
            ref={charWrapRef}
            className="absolute flex justify-center pointer-events-none"
            style={{
              bottom: "-0.05em", // Fine-tunes the feet to sit perfectly on the font baseline
              height: "1.1em", // Scales perfectly with the font size clamp
              zIndex: 20,
            }}
          >
