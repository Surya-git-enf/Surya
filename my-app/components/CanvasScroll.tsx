...
export default function CanvasScroll() {
  ...
  useLayoutEffect(() => {
    ...
    // All cards start fully hidden and far below
    ...
    const ctx = gsap.context(() => {
      /*
        Timeline beats (each "1 unit" ≈ one scroll segment):
          0 → 1   : Intro text fades in
          1 → 1.8 : Intro text fades out
          1.6 → 2.4 : Card 1 rises in
          2.2 → 3.0 : Card 2 rises in (overlaps slightly)
          2.8 → 3.6 : Card 3 rises in (overlaps slightly)
          3.6 → 4.6 : All three held on screen — user can read
          Total timeline: ~4.6 units → set end to +=460%
      */
      const tl = gsap.timeline({
        scrollTrigger: {
          trigger: container,
          start: "top top",
          // Each unit ≈ 100vh of scroll space. 5 units = 500vh pinned scroll.
          end: "+=500%",
          pin: true,
          scrub: 1.2,
          anticipatePin: 1,
      ...
      // 3. Card 1 rises in — solo, front and centre
      ...
      // 6. All three cards gently float up a little (parallax drift) while held
      ...
      // 7. Hold — empty buffer so the user sees all three cards before unpin
      tl.set({}, {}, 5.0);
  ...
  return (
    /* The outer div provides the scroll-trigger pin space */
    <div ref={containerRef} className="relative w-full overflow-hidden bg-black" style={{ height: "100vh" }}>
      ...
      <div className="absolute inset-0 z-30 flex flex-col items-center justify-center pointer-events-none">
        ...
        {/* Cards — stacked vertically on mobile, row on desktop */}
